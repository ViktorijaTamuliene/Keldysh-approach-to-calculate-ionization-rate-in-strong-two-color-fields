...
c1=sqrt(2*P.I0/K.h*P.w1)/beta^2/(-1+beta^2)/((g1+g2)^2+4*g2)*(-1)^n1*sqrt(2*(n1-I0s/K.h/P.w1));
...
c3x=exp(beta/sqrt(-1+beta^2)*(2*(n1-I0s/K.h/P.w1))*(x.^2-g2/g1*(-1+beta^2)*(1-x.^2)/2)/(1+g2/g1*(1-2*beta^2))   )...
    .*besseli(m,beta*sqrt(-1+beta^2)*g2/g1*(n1-I0s/K.h/P.w1)*(1-x.^2)/(1+g2/g1*(1-2*beta^2)))...
    .*besselj(2*m,4*(n1-I0s/K.h/P.w1)*sqrt(g2/g1)*x.*sqrt(1-x.^2)/(1+g2/g1*(1-2*beta^2)));
c3=c3+trapz(x,c3x);
